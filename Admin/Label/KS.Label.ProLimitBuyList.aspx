<%@ Page Language="C#" MasterPageFile="~/Admin/MasterPage.master" AutoEventWireup="true" Inherits="Kesion.NET.WebSite.Admin_Label_ProLimitBuyList" Title="Untitled Page" Codebehind="KS.Label.ProLimitBuyList.aspx.cs" %>
...
<script type="text/javascript">
  ...
  {
    jQuery.get('KS.GetFields.aspx',{channelid:channelid,basictype:4,islimitbuy:1},function(s){
  ...
  function checkform()
  {
   ...
   var taskid = '';
   jQuery("#<%=this.LstTaskID.ClientID %> option:selected").each(function() {
       if (jQuery(this).val() != '0')
           if (taskid == '')
               taskid = jQuery(this).val()
       else
           taskid += "," + jQuery(this).val();
   })
   if (taskid == '') taskid = '0';
   ...
   var str='{KS:Label Type="IProLimitBuyList"'
...
<KS:TopNav runat="server" Text="创建限时/限量商品列表式标签第一步" ID="TopNav1" />
    ...
    <KS:Foot ID="Foot1" runat="server" OnSubmit="Foot1_Submit" SubmitButtonText="下一步(0)" />
        ...
        <asp:Panel ID="Step2Panel" runat="server" Visible="false" Width="100%">
            ...
            <tr>
                ...
                <td class="lefttd" style="height: 30px; text-align: right">
                    <strong>指定抢购任务：</strong></td>
                <td class="righttd" style="height: 30px; text-align: left">
                    <asp:ListBox ID="LstTaskID" runat="server" Rows="3" SelectionMode="Multiple" 
                        Width="120">
                        <asp:ListItem Value="">-不指定任务-</asp:ListItem>
                    </asp:ListBox>
                      <span class="tips">可以按Ctrl多选多个抢购任务，也可以不选。</span>
                </td>
            </tr>
            <tr>
                <td class="lefttd" style="height: 30px; text-align: right">
                ...
                <td id="Td3" runat="server" class="righttd" colspan="2" 
                    ...
                    可留空，如: and I.Inputer=&#39;admin&#39;
                </td>
               
            </tr>
                    <tr>
                        <td class="lefttd" style="height: 30px; text-align: right">
                            <strong>所属栏目：</strong><br />
                        </td>
                        <td ID="Td4" runat="server" class="righttd" colspan="2" 
                            style="height: 30px; text-align: left">
                            <table border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                    <td style="width: 1px">
                                        <select ID="classid1" multiple name="classid1" size="3" style="width: 120px">
                                            <option value="0">-不指定栏目-</option>
                                            <option style="color:red" value="-1">当前栏目通用</option>
                                            <%=MyCache.GetShopClassOptionList(modelid)%>
                                        </select>
                                    </td>
                                    <td valign="top">
                                        <asp:CheckBox ID="ChildClass" runat="server" Checked="true" 
                                            Text="允许列出子栏目商品(仅对单选有效)" />
                                        <br />
                                        &nbsp;&nbsp;<font color="blue">若要调用多个栏目，可以按Ctrl进行多选</font>
                                    </td>
                                    <td nowrap>
                                        <strong>所属品牌：</strong>
                                    </td>
                                    <td valign="top">
                                        <asp:ListBox ID="LstBrandID" runat="server" Rows="3" SelectionMode="Multiple" 
                                            Width="120">
