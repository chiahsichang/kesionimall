<%@ Page Title="相册管理" Language="C#" MasterPageFile="~/Admin/MasterPage.master" AutoEventWireup="true" Inherits="Kesion.NET.WebSite.Admin_BlogMessage" Codebehind="KS.BlogMessage.aspx.cs" %>
<%@ Import Namespace="Kesion.Publics"%>
<asp:Content ID="Content1" ContentPlaceHolderID="KSContent" Runat="Server">

    <asp:Panel ID="listPanel" runat="server">

<KS:TopNav ID="TopNav1" runat="server" Text="空间留言管理" />
               ...
               <td style="height: 23px">
                  <b>快速搜索：</b> <asp:TextBox onfocus="keyWordFocus();" onblur="keyWordBlur();" style="color:#ccc" Text="请输入关键字！" CssClass="textbox" ID="txtKeyWord" runat="server"></asp:TextBox>&nbsp;条件：
                   <asp:DropDownList ID="dropSearch" runat="server">
                      <asp:ListItem Value="1">留言者用户名</asp:ListItem>
                      <asp:ListItem Value="2">留言标题</asp:ListItem>
                      <asp:ListItem Value="3">留言内容</asp:ListItem>
                   </asp:DropDownList>
...
<asp:Panel ID="Panel1" runat="server">
    <div class="tabs_header">
        <ul class="tabs">
        <li<%=KSCMS.S("showType")==""?" class='active'":string.Empty %>><a href="KS.BlogMessage.Aspx"><span>所有记录</span></a></li>
        <li<%=KSCMS.S("showType")=="1"?" class='active'":string.Empty %>><a href="KS.BlogMessage.Aspx?showType=1"><span>未审核的记录</span></a></li>
        <li<%=KSCMS.S("showType")=="2"?" class='active'":string.Empty %>><a href="KS.BlogMessage.Aspx?showType=2"><span>已审核的记录</span></a></li>
        </ul>
    </div>

    <KS:ksgv id="List" cssclass="CTable" runat="server" AutoGenerateColumns="False" EmptyDataText="没有留言记录！" PagerType="CustomNumeric" Width="99%" GridLines="None" OnRowDataBound="List_RowDataBound" OnRowCommand="List_RowCommand" OnRowCreated="List_RowCreated" >
            <Columns>
                ...
                <asp:TemplateField HeaderText="标题" ItemStyle-Width="400" ItemStyle-HorizontalAlign="Left"  >
                    <ItemTemplate>
                        <%# (bool)string.IsNullOrEmpty(Eval("Title").ToString())? "(无)": "<a href='"+KSCMS.GetInstallDir()+"space/?"+Eval("UserId")+"/message' target='_blank'>"+Eval("Title")+"</a>"%>
                    </ItemTemplate>
                </asp:TemplateField>



                <asp:BoundField HeaderText="留言时间" DataField="AddDate" >
              ...
              <itemtemplate>
                   <a href="?action=check&ID=<%#Eval("ID")%>" <%# Convert.ToString(Eval("Flag"))=="0"?"class='no' title='未审核'":"class='ok' title='已审核'" %>></a>
              </itemtemplate>
            </asp:TemplateField>
            ...
            <asp:TemplateField HeaderText="查看">
                       <itemstyle horizontalalign="Center" width="25px"/>
                        <itemtemplate>
                 <asp:LinkButton ID="LinkButton2" CssClass="edit" ToolTip="查看" runat="server" commandargument=<%#Eval("ID") %> commandname="detail"><img src="../../admin/images/Edit.gif" border="0" title="查看" />查看</asp:LinkButton> &nbsp;&nbsp;
                 </itemtemplate>
              </asp:TemplateField>
