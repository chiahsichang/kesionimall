<%@ Page Title="相册管理" Language="C#" MasterPageFile="~/Admin/MasterPage.master" AutoEventWireup="true" Inherits="Kesion.NET.WebSite.Admin_BlogComment" Codebehind="KS.BlogComment.aspx.cs" %>
<%@ Import Namespace="Kesion.Publics"%>
<asp:Content ID="Content1" ContentPlaceHolderID="KSContent" Runat="Server">

    <asp:Panel ID="listPanel" runat="server">
<div id="manage_top">
       <p style="text-align:center;font-weight:bold;">日志评论管理</p>
</div> 
...
<asp:Panel ID="Panel1" runat="server">
    ...
    <KS:ksgv id="List" runat="server" AutoGenerateColumns="False" EmptyDataText="无记录" PagerType="CustomNumeric" Width="99%" CssClass="CTable" GridLines="None" OnRowDataBound="List_RowDataBound" OnRowCommand="List_RowCommand" OnRowCreated="List_RowCreated" >
            <Columns>
                ...
                <asp:TemplateField HeaderText="标题" ItemStyle-Width="400" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Left" >
                    <ItemTemplate>
                        <%# (bool)string.IsNullOrEmpty(Eval("Title").ToString())? "(无)": "<a href='"+KSCMS.GetInstallDir()+"space/?"+Eval("UserId")+"/blog/archive/"+Eval("LogID")+"' target='_blank'>"+Eval("Title")+"</a>"%>
                    </ItemTemplate>
                </asp:TemplateField>
                ...
                <asp:TemplateField HeaderText="↓管理操作">
                 ...
                 <asp:LinkButton ID="LinkButton2" runat="server" commandargument=<%#Eval("ID") %> commandname="detail" CssClass="preview" ToolTip="查看"></asp:LinkButton> &nbsp;&nbsp;
                 <asp:LinkButton ID="LinkButton1" runat="server" commandargument=<%#Eval("ID") %> commandname="check" CssClass="verify" ToolTip="审核"></asp:LinkButton> &nbsp;&nbsp;
                 <asp:LinkButton runat="server" id="delete" commandargument=<%#Eval("ID") %> commandname="del" CssClass="delete" ToolTip="删除"></asp:LinkButton>
