<%@ Page Language="C#" MasterPageFile="~/Admin/MasterPage.master" AutoEventWireup="true" Inherits="Kesion.NET.WebSite.Admin_ContentBatch" Title="批量设置信息属性" Codebehind="KS.ContentBatch.aspx.cs" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="KSContent" Runat="Server">
   ...
   <td vAlign=top>
				  ...
				  <tr>
				    <td align='center' class='lefttd' style="width: 52px;  height: 30px;"><asp:CheckBox ID="C1" runat="server" /></td>
					<td align='right' class='lefttd' style="width: 105px; height: 30px;"><strong>
					<%=ItemName %>作者：</strong></td>
					<td class="righttd" style="height: 30px">
                        <asp:TextBox CssClass="textbox" ID="Author" runat="server"></asp:TextBox>
                        
                        <<【<font color='blue'><font color='#993300' onclick='jQuery("#<%=this.Author.ClientID %>").val("未知");' style='cursor:hand;'>未知</font></font>】【<font color='blue'><font color='#993300' onclick="jQuery('#<%=this.Author.ClientID %>').val('佚名');" style='cursor:hand;'>佚名</font></font>】
				<select name='SelAuthor' style='width:100px' onChange="jQuery('#<%=this.Author.ClientID %>').val(this.options[this.selectedIndex].value);">")
		    <option value="" selected> </option><option value="" style="color:red">清空</option>
			<%=BaseFun.GetSQLOption("select top 100 AuthorName,AuthorName From KS_Author order by authorid desc", "") %>
			 </select>
                        </td>          
				  </tr>
                      <tr>
                          ...
                          <td align="right" class="lefttd" style="width: 105px; height: 30px;">
                              <strong><%=ItemName %>来源<span style="background-color: #f0f9fe">：</span></strong></td>
                          <td class="righttd" style="height: 30px">
                              <asp:TextBox CssClass="textbox" ID="CopyFrom" runat="server"></asp:TextBox>
                          <<【<font color='blue'><font color='#993300' onclick="jQuery('#<%=this.CopyFrom.ClientID %>').val('不详');" style='cursor:hand;'>不详</font></font>】【<font color='blue'><font color='#993300' onclick="jQuery('#<%=this.CopyFrom.ClientID %>').val('本站原创');" style='cursor:hand;'>本站原创</font></font>】【<font color='blue'><font color='#993300' onclick="jQuery('#<%=this.CopyFrom.ClientID %>').val('互联网')" style='cursor:hand;'>互联网</font></font>】
			<select name='selOrigin' style='width:100px' onChange="jQuery('#<%=this.CopyFrom.ClientID %>').val(this.options[this.selectedIndex].value)">
		    <option value="" selected> </option><option value="" style="color:red">清空</option>
			<%=BaseFun.GetSQLOption("select top 100 CopyFromName,CopyFromName From KS_CopyFrom order by copyfromid desc", "") %>
			</select>    
                          </td>
                      </tr>
				  <tr>
				    <td align='center' class='lefttd' style="width: 52px"><asp:CheckBox ID="C3" runat="server" /></td>
					<td height='30' align='right' class='lefttd' style="width: 105px"><strong>点 击 数：</strong></td>
					<td height='28' class="righttd">
                        <asp:TextBox CssClass="textbox" ID="Hits" runat="server" Text="100" Width="57px"></asp:TextBox>
            ...
            {%>
				      ...
				      <%} %>
					  <input class="textbox" type="text" name='ContentTemplate' id="ContentTemplate" size='30'>&nbsp;<input type='button' name="Submit" class="button" onmouseover="this.className='button_on';" onmouseout="this.className='button';" value="选择模板..." onClick="SelectTemplate('ContentTemplate');">    
                      </td>
                   </tr>  
