<%@ Page Language="C#" MasterPageFile="~/Admin/MasterPage.master" AutoEventWireup="true" Inherits="Kesion.NET.WebSite.Admin_ShopRecycleBin" Title="回收站管理" Codebehind="KS.ShopRecycleBin.aspx.cs" %>
...
function keyWordBlur() {
  ...
  //-->    
   ...
   <div id="manage_top" class="toptitle menu_top_fixed" style="text-align:left">
    <span style="float:right;padding-right:10px;"><select name="classid" style="width:150px" OnChange="location.href='KS.ShopRecycleBin.Aspx?classid='+this.value;">
                <option value=0>快速跳转到栏目...</option>
                <%=ClassManage.GetClassOptionByPurview(Convert.ToInt16(ChannelID),ClassID.ToString())%>
               </select>  </span>    
   
   回收站管理[共有<span style="color:Red"><%=totalNum%></span>件商品]</div>
 ...
 <div class="message">
    ...
    <KS:KSGV ID="List" cssclass="CTable" runat="server" AutoGenerateColumns="False" PagerType="CustomNumeric"
        Width="99%" OnRowCommand="list_RowCommand" EmptyDataText="回收站中没有商品!" HorizontalAlign="Center" OnRowDataBound="list_RowDataBound" GridLines="None" OnRowCreated="List_RowCreated">
        <Columns>
            ...
            <asp:TemplateField HeaderText="标题">
                ...
                [<%# MyCache.GetCacheClass(Convert.ToInt32(Eval("classid")),1) %>]
                 <a href="../../model/view.aspx?m_id=<%#Eval("ChannelID")%>&id=<%#Eval("InfoID") %>" target="_blank" title='<%# Eval("Title") %>'><%# Utils.CutStr(Convert.ToString(Eval("Title")),52) %></a>
                </itemtemplate>
            </asp:TemplateField>
    ...
    <asp:Panel ID="Panel1" runat="server" Height="30px">
          ...
          <asp:Button ID="Button1" runat="server" Text="批量还原" onclick="Button1_Click1" />&nbsp;
            <input type="button" value="一键清空" onMouseOver="this.className='button_on';" onMouseOut="this.className='button';" class="button" onClick="KesionJS.Confirm('此操作不可逆，确定一键清空回收站里的所有商品吗?','location.href=\'<%=KSCMS.GetInstallDir()+MyCache.GetCacheConfig(4) %>/shop/KS.ShopRecycleBin.aspx?action=delall\';','');"/>
