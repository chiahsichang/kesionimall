...
function fileQueued(file) {
	...
	try {
		...
		progress.setStatus("<img src='../../sysimg/default/ico_open.gif' align='absmiddle'> 等待上传...");
...
function createtr(ffile){
	...
	var str='<tr id="tr'+ffile.id+'"><td class="splittd" nowrap style="padding-left:5px"><img src="../../sysimg/FileIcon/'+ext+'.gif" align="absmiddle"/> <strong id="f'+ffile.id+'">'+ffile.name.substring(0,26)+'</strong></td><td class="splittd" style="text-align:center" width="100">'+(ffile.size/1024).toFixed(2)+'KB<span style="display:none" id="size'+ffile.id+'">'+ffile.size+'</span></td><td class="splittd" width="260" id="p'+ffile.id+'">等待上传...</td><td class="splittd" style="text-align:center" id="info'+ffile.id+'"><img style="cursor:hand" sid="'+ffile.id+'" name="cdel" border="0" src="../../sysimg/default/close.gif"/></td></tr>';
...
function uploadStart(file) {
	hastr=true;
	this.addPostParam('aliasName',file.name);   
	this.addPostParam('dSize',file.size);   
	try {
		 ...
		 */
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setStatus("<img src='../../sysimg/default/loading.gif' align='absmiddle'> 正在上传中...");
		progress.toggleCancel(true, this);
	}
	catch (ex) {}
...
function uploadSuccess(file, serverData) {
	try {
		if (serverData.substring(0, 6) == "error:") {
			...
			if (basictype!=99999){ parent.InsertFileFromUp(d[0],file.size,d[1],file.name,d[2])}
		 }
		var progress = new FileProgress(file, this.customSettings.progressTarget);
