<%@ Page Language="C#" AutoEventWireup="true" Inherits="Kesion.NET.WebSite.UserUpload1" Codebehind="Upload.aspx.cs" %>
...
<script runat="server">
    ...
    string fieldId = "-1";
    int formId = Utils.StrToInt(KSCMS.S("formid"));
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      if (channelId>0 && channelId!=1001 && channelId!=1002)
      {
   ...
   <%if (upBySwf == 1)  //用swfupload 
  {
     %>
      ...
      <% if (!string.IsNullOrEmpty(module))  //XML配置上传
      {
        %>
          ...
          <%
			...
			var fileExt="<%=Utils.getSwfUploadExt(string.Concat(__info.AllowUpFlashType, "|", __info.AllowUpMediaType, "|", __info.AllowUpOtherType, "|", __info.AllowUpPhotoType, "|", __info.AllowUpRealType)) %>" //限制扩展名
			 ...
			 <%} %>
          <%}
            else if (fieldId.ToLower() == "affix")
            {
                Kesion.Components.BlogConfigInfo __configinfo = Kesion.Config.BlogConfigManage.Deserialize(Server.MapPath("~/config/Blog.config"));
                string fileSite = __configinfo.BlogArticleFileSize;
                string fileExt = __configinfo.BlogArticleFileExt;
		 %>
          var limitSize=<%=fileSite%>;
          var fileExt="<%=Utils.getSwfUploadExt(fileExt) %>" //限制扩展名

          <%}else if (fieldId.ToLower()=="defaultpic" || isPic==1){
		 %>
		 	var limitSize=<%=__info.AllowUpFilesSize%>; //限制大小 KB
			var fileExt="<%=Utils.getSwfUploadExt(__info.AllowUpPhotoType) %>" //限制扩展名
       <%}else if (channelId==1001){ //会员注册时的上传字段 
            XmlDocument xmldoc = new XmlDocument();
            xmldoc.Load(Server.MapPath(KSCMS.GetInstallDir()+"config/userfield.config"));
            string FieldsXML = XmlObject.XMLToString(ref xmldoc);
       ...
       %>
         ...
         <%}else if (channelId==1002){ //自定义表单
            if (formId==0){
             KSCMS.Die("error");
            }else{
             FormInfo __info = FormManage.GetFormSubjectInfo(formId);
             if (__info==null){
             KSCMS.Die("error");
             }else{
             ArrayList al = new ArrayList();
             al = XmlObject.GetSubElementByAttributeFromStr(__info.FieldsXML, "field", "@name", fieldId.Split('_')[0]);
            %>
		 	var limitSize=<%=Utils.StrToInt(al[7].ToString())%>; //限制大小 KB
			var fileExt="<%=Utils.getSwfUploadExt(al[6].ToString()) %>" //限制扩展名
            <%
             }
            }
         }else{ %>
     ...
     <%if (UpType == "UpByBar")  //上传（带进度条）
       ...
       <script>
			...
			var post_params={"username":"<%=KSCMS.GetCookie("User","username") %>","password":"<%=KSCMS.GetCookie("User","password") %>","upBySwf":1,ChannelID:<%=channelId%>,"formid":<%=formId%>,"UpType":"<%=UpType%>","FieldID":"<%=fieldId%>","CurrPath":"<%=CurrPath %>","AutoRename":4};
			var buttonstyle="color:#000000;";
					...
					if (serverData.substring(0, 6) == "error:") {
						alert(unescape(serverData).replace("error:",""));
					     ...
					     <%if (fieldId=="-1"){
					       ...
					       <%}else{ %>
					        parent.SetDownUrlByUpLoad(serverData);  //下载的软件
					     <%}
					     }else{ %>
      ...
      %>
		...
		function uploadSuccess1(file, serverData) {
			try {
				if (serverData.substring(0, 6) == "error:") {
                  ...
                  {%>
				    $(function(){
				        $("input[affix='affixed']", window.parent.document).val(serverData);
				        $(".txtuploadfile2", window.parent.document).val(file.size + "," + file.type + "," + file.name);
				        $("input[affix='affixed']", window.parent.document).focus();
				        $("div[affix='divaffixed']", window.parent.document).html(serverData);
				        $(".txtuploadfile2", window.parent.document).focus();
				        console.info(file);
				    });
		...
		window.onload = function () {
			...
			swfu = new SWFUpload({
				...
				post_params: {"username":"<%=KSCMS.GetCookie("User","username") %>","password":"<%=KSCMS.GetCookie("User","password") %>","upBySwf":1,UpType:"<%=UpType%>",module:"<%=module%>",ChannelID:<%=channelId%>,"FieldID":"<%=fieldId%>","formid":<%=formId%>,"ispic":<%=isPic%>,"AutoRename":4,"AddWaterMark":<%=AddWaterMark%>,"CreateThumbs":<%=CreateThumbs%><%if (isPic==1){ %>,"AddWaterMark":1,"CreateThumbs":1<%} %>},
				...
				button_text_style : '.button { line-height:<%=btnHeight%>px;font-family: Helvetica, Arial, sans-serif;color:#555555;} ',
		...
		<table cellspacing="0" cellspadding="0" border="0">
		 ...
		 <%
           ...
           if (isPic == 1)
      {%>
		  <td><label><input type="checkbox" name="DefaultUrl" id="DefaultUrl" onClick="SetDefaultUrl(this)" value="1" checked>自动生成缩略图</label>
		  
		   <%if (ConfigManage.GetConfig().WaterMarkStatus!=0){ %>     
               <label><input name="AddWaterFlag" type="checkbox" id="AddWaterFlag" onclick="SetAddWaterFlag(this)" value="1" checked>添加水印</label> 
              <%} %>
