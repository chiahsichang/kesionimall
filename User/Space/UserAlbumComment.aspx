<%@ Page Title="" Language="C#" MasterPageFile="~/User/User.master" AutoEventWireup="true" Inherits="Kesion.NET.WebSite.User_UserAlbumComment" Codebehind="UserAlbumComment.aspx.cs" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="KSContent" Runat="Server">
    ...
    <div class="tabs_header">
        <ul class="tabs">
        <li<%=Kesion.Publics.KSCMS.S("showType")==""?" class='puton'":string.Empty %>><a href="UserAlbumComment.Aspx"><span>所有记录</span></a></li>
        <li<%=Kesion.Publics.KSCMS.S("showType")=="1"?" class='puton'":string.Empty %>><a href="UserAlbumComment.Aspx?showType=1"><span>未审核的记录</span></a></li>
        <li<%=Kesion.Publics.KSCMS.S("showType")=="2"?" class='puton'":string.Empty %>><a href="UserAlbumComment.Aspx?showType=2"><span>已审核的记录</span></a></li>
        </ul>
    </div>
    ...
    <KS:ksgv id="List" runat="server" CssClass="border" AutoGenerateColumns="False" EmptyDataText="无记录" PagerType="CustomNumeric" Width="99%" HorizontalAlign="center" GridLines="None" OnRowDataBound="List_RowDataBound" OnRowCommand="List_RowCommand" OnRowCreated="List_RowCreated" >
            <Columns>
                ...
                <asp:TemplateField HeaderText="标题" ItemStyle-CssClass="splittd" ItemStyle-Width="400" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Left" >
                    <ItemTemplate>
                        <%# (bool)string.IsNullOrEmpty(Eval("Title").ToString())? "(无)": "<a href='"+GetBlogUrl(Eval("userid").ToString(), Eval("LogID").ToString())+"' target='_blank'>"+Eval("Title")+"</a>"%>
                    </ItemTemplate>
                </asp:TemplateField>

                <asp:BoundField HeaderText="留言时间" DataField="AddTime" ItemStyle-CssClass="splittd">
    ...
    <asp:Panel runat="server" ID="Mdatil" Visible="false">
        ...
        <p style="text-align:center;"><asp:Button ID="Button1" runat="server" OnClientClick="return CheckForm()" class="button" text=" 回 复 " OnClick="Unnamed1_Click" />　　 <asp:Button runat="server" class="button" text=" 返 回 " OnClick="Unnamed1_Click2" /></p>
        </form>
    </asp:Panel>
